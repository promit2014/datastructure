 ...
 * If it is MAX heap then it means the root is the max value in the tree.
 * MIN tree is just opposite to MAX heap.
 *
 * Heap is tree which is represented by an Array instead of a node class.
...
public class Heap<T extends Comparable<T>> {
    ...
    /**
     * This function recursively check the value of the parent with the cuurent node,
    ...
    /**
     * This functions return swaps the valyes of the two indexes passes with the list
    ...
    /**
     * This is a method that is used for converting a binary tree in array representation to a Heap/PriorityQueue
     *
     */
    public void heapify(){
        if(list.size()<=0){
    ...
    /**
     * This is a recursive method that swaps the value of the minimum number with the value of the child nodes
     * except the leaf nodes
     * @param index
     */
    private void heapify(int index){
        if(getLeftChildIndex(index) > list.size()-1 && getRightChildIndex(index) > list.size()-1){
            return ;
        }
        ...
        if(rightChildIndex <= list.size()-1 && list.get(rightChildIndex).compareTo(list.get(index))<0){
            minIndex = rightChildIndex;
        }
        if(leftChildIndex <= list.size()-1 && list.get(leftChildIndex).compareTo(list.get(minIndex))<0){
            minIndex = leftChildIndex;
        }
        swapByIndex(list, index, minIndex);
    }
